<?php  
    ...
    $query = mysqli_query($koneksi, "SELECT MAX(no_order) from permintaan");
    $no_order = mysqli_fetch_array($query);    
    if ($no_order) {
            
            $nilaikode = substr($no_order[0], 2);
            
            $kode = (int) $nilaikode;

            //setiap kode ditambah 1
            $kode = $kode + 1;
            $kode_otomatis = "OR".str_pad($kode, 6, "0", STR_PAD_LEFT);                   
            
        
    } else {
         $kode_otomatis = "OR0000001";
    }
    ...
    <div class="row">
        <div class="col-sm-6 col-xs-12">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="text-center">Form Order Permintaan Barang</h3>
                </div>
                <form method="post" id="tes"  action="add-proses.php" class="form-horizontal">
                    <div class="box-body">
                    	<div class="form-group ">
                            <label for="no_order" class="a col-sm-3 control-label">No. Order</label>
                            <div class="col-sm-3">
                                <input type="text" readonly value="<?=$kode_otomatis; ?>" class="form-control" name="no_order">
                            </div>
                        </div>
                        <div class="form-group ">
                            <label for="nama_brg" class="a col-sm-3 control-label">Bioskop</label>
                            <div class="col-sm-3">
                        ...
                        <!-- <div class="form-group ">
                            <label for="nama_brg" class="a col-sm-3 control-label">Bioskop</label>
                            <div class="col-sm-3">
                                <select id="unit" required="isikan dulu" class="form-control" name="unit">
                                <option value="">--Pilih Bioskop--</option>
                                <?php  
                                    include "../fungsi/koneksi.php";
                                    $queryBio = mysqli_query($koneksi, "select * from bioskop");
                                    if (mysqli_num_rows($queryBio)) {
                        ...
                        <div class="form-group">
                            ...
                            &nbsp;
                            <a href="index.php?p=datapesanan" class="btn btn-warning"><span>Kembali</span></a>
        ...
        <div class="col-sm-6 col-xs-12">
            <div class="box box-info">
                <div class="box-header with-border">
                    <h3 class="text-center">Data Order Hari Ini</h3>
                </div>
                    ...
                    <table class="table table-responsive">
                        ...
                        <?php 
                            $sekarang  = date("Y-m-d");
                            $queryTampil = mysqli_query($koneksi, "SELECT sementara.unit, sementara.no_order, sementara.status, sementara.id_sementara, stokbarang.nama_brg, stokbarang.satuan, sementara.keterangan, jumlah FROM sementara INNER JOIN stokbarang ON sementara.kode_brg  = stokbarang.kode_brg WHERE tgl_permintaan = '$sekarang' AND sementara.unit='$_SESSION[username]' "  );
                            $no = 1;
                    ...
                    <?php $no++; endwhile; } else { echo "<tr><td>Tidak ada order barang hari ini</td></tr>"; } ?>
                    </table>    
                <div class="box-body">
                    <a class="btn btn-success" href="pesan.php" >Order Barang</a>
                </div>
            </div>
        </div>
    </div>
