...
<?php  
    ...
    $query = mysqli_query($koneksi, "SELECT permintaan.id_permintaan, no_order, permintaan.kode_brg, tgl_permintaan, unit, 
                                        nama_brg, jumlah, satuan,  
                                        `status`, bioskop.nama_bioskop, bioskop.alamat 
                                        FROM permintaan 
                                        INNER JOIN stokbarang ON permintaan.kode_brg = stokbarang.kode_brg 
                                        INNER JOIN `user` ON `user`.username=permintaan.unit
                                        INNER JOIN `bioskop` ON bioskop.kode_bioskop=`user`.id_bioskop
                                        WHERE STATUS=1 ORDER BY tgl_permintaan DESC"); 
